 ...
 * @icon fa fa-circle-o
 */
class Ribao extends Backend
{

    /**
     * Ribao模型对象
     * @var \app\admin\model\laoban\Ribao
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\laoban\Ribao;
    ...
    public function index()
    {
        ...
        $list = $this->model
            ...
            foreach ($list as $key=>$value) {
                $admin_info = Db::name('admin')->field('nickname')->where(['id'=>$value['admin_id']])->find();
                if($admin_info){
                     $list[$key]['admin_id'] = $admin_info['nickname'];
                }else{
                    $list[$key]['admin_id'] = "用户信息不存在";
 ...
 /**
    ...
    public function add()
    {
        if (false === $this->request->isPost()) {
            $zuidashijian = date("Y-m-d 23:59:59",time());
            $this->view->assign('zuidashijian', $zuidashijian);
            return $this->view->fetch();
        }
        ...
        $find_ribao = Db::name('ribao')->field('id')->where($find_where)->find();
        ...
        if($find_ribao){
            ...
            Db::name('ribao')->field('id')->where($find_where2)->delete();
        }
        $this->success();
    }
