...
class Daka extends Backend
{
     ...
     * 快速搜索时执行查找的字段
     */
    protected $searchFields = 'admin_id';
    ...
    protected function buildparams($searchfields = null, $relationSearch = null)
    {
        ...
        $limit = $this->request->get("limit/d", 999999); // 设置很大的值，实际不分页
        //新增自动计算页码
    ...
    public function index()
    {
        ...
        $search = $this->request->get("search", ''); // 顶部搜索框的参数
        $chuqinstatusSearch = ''; // 用于搜"出勤状态"
        $chuqinlistSearch   = ''; // 用于搜"出勤记录"

        // 优先处理顶部搜索框的 search 参数
        if ($search !== '') {
            // 通过员工名称（nickname）查找用户
            $user_info = Db::name('admin')
                ->where(['nickname' => $search])
                ->find();
            if (!$user_info) {
                $this->error("未查询到员工" . $search . "的信息");
            }
            $year  = date('Y');
            $month = date('m');
        } elseif ($filter) {
            ...
            $user_info = Db::name('admin')
                ->where(['nickname' => $filterArr['admin_id']])
                ->find();
            if (!$user_info) {
            ...
            // 没有传 filter 和 search，默认查当前登录用户+当年当月
            $year  = date('Y');
            $month = date('m');
        ...
        // 根据用户的 group_id 查找对应的考勤配置，使用 FIND_IN_SET 支持多部门公用（如 group_ids="1,2"）
        // FIND_IN_SET 可以在逗号分隔的字符串中查找指定值
        $attendance_info = Db::name('attendance')
            ->whereRaw("FIND_IN_SET(?, group_ids)", [$group_id])
            ->find();
        ...
        // 通过 attendanceshift_id 关联查询 kao_attendanceshift 表获取 starttime 和 endtime
        $shift_starttime = null;
        $shift_endtime = null;
        if (!empty($attendance_info['attendanceshift_id'])) {
            $shift_info = Db::name('attendanceshift')
                ->where(['id' => $attendance_info['attendanceshift_id']])
                ->find();
            if ($shift_info) {
                $shift_starttime = $shift_info['starttime'];
                $shift_endtime = $shift_info['endtime'];
            }
        }
        ...
        foreach ($allDays as $d) {
            ...
            foreach ($approve_info as $ap) {
                $ap_start = strtotime($ap['starttime']);
                $ap_end   = strtotime($ap['endtime']);
                // 只要当天与请假区间有交集
            ...
            // ---------- (D) 工作日 + 未请假 => 判断上班 / 下班 ----------
            // 使用从 kao_attendanceshift 表获取的 starttime 和 endtime
            if ($shift_starttime !== null && $shift_endtime !== null) {
                $expectedStartTimeStr = $d . " " . date("H:i:s", $shift_starttime);
                $expectedEndTimeStr   = $d . " " . date("H:i:s", $shift_endtime);
            } else {
                // 如果未找到班次信息，使用原来的逻辑（兼容处理）
            ...
            $dayRecord['dakatime'] = $expectedStartTimeStr;
            if ($recordMorning) {
            ...
            $eveningRecord['dakatime'] = $expectedEndTimeStr;
            if ($recordEvening) {
                ...
                $eveningRecord['typelist']     = '2'; // 2=下班打卡
            }
        ...
        // =======================
        // 注意：顶部搜索框的 search 参数已经在步骤1中处理，通过查找用户来过滤数据
